...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        added = False
        if player.hand != []:
            player.hand.insert(len(player.hand), player.hand[0].copy())
            added = True
        # You should add your implementation above this.
        if added:
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if player.hand != []:
            for i in range(len(player.hand)):
                if player.hand[i].power(opponent_card) > player.hand[highest].power(opponent_card):
                    highest = i
            self.attack += player.hand[highest].attack
            self.defense += player.hand[highest].defense
            best_card = player.hand[highest]
            player.hand.remove(player.hand[highest])
        # You should add your implementation above this.
        if best_card:
