...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        additionaltree = []
        for i in leaves:
            additionaltree.append(tree(i))
        return tree(label(t), additionaltree)
    else:
...
def add_trees(t1, t2):
    ...
    """
    "*** YOUR CODE HERE ***"
    #I used imports from python itertools
    if is_leaf(t1) and is_leaf(t2):
        return tree(label(t1)+label(t2))
    if is_leaf(t1):
        return tree(label(t1)+label(t2), [i for i in branches(t2)])
    if is_leaf(t2):
        return tree(label(t1) + label(t2), [i for i in branches(t1)])
    return tree(label(t1)+label(t2), [add_trees(i, j) for i,j in zip_longest(branches(t1), branches(t2), fillvalue=tree(0))])
